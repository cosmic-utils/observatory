...
impl Application {
    ...
    pub fn scan_all() -> Vec<Application> {
        ...
        let files = std::fs::read_dir("/usr/share/applications")
            .expect("Unable to read /usr/share/applications!");
        for file in files {
            ...
            if let Ok(desktop_entry) = parse_entry(&path) {
                let name = if let Some(name) = desktop_entry.section("Desktop Entry").attr("Name") {
                    ...
                    path.file_name()
                        .expect("Could not read file name of application!")
                        .to_str()
                        .unwrap()
                        .to_string()
                };
                ...
                let full_cmd =
                    if let Some(cmd) = desktop_entry.section("Desktop Entry").attr("Exec") {
                        cmd.to_string()
                    } else {
                        "".to_string()
                    };
