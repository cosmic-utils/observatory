use cosmic::{widget, theme, Element};
...
impl Process {
    pub fn from_process(categories: &CategoryList, process: &sysinfo::Process, apps: &Vec<Application>, sys: &sysinfo::System, users: &sysinfo::Users) -> Self {
    ...
    pub fn compare(a: &Process, b: &Process, cat: &Category) -> std::cmp::Ordering {
        match cat {
            Category::Name => {
                let mut ord = b.data_points[Category::Name.index() as usize].data.to_ascii_lowercase().cmp(&a.data_points[Category::Name.index() as usize].data.to_ascii_lowercase());
            ...
            Category::User => {
                let mut ord = a.data_points[Category::User.index() as usize].data.cmp(&b.data_points[Category::User.index() as usize].data);
                if ord == std::cmp::Ordering::Equal {
    ...
    pub fn element(&self, theme: &cosmic::Theme, is_selected: bool) -> Element<Message> {
        let row = widget::row::with_children::<Message>(self.data_points.iter().map(|dp|
            dp.element(theme)
        ).collect());
...
impl DataPoint {
    fn new(category: Category, process: &sysinfo::Process, apps: &Vec<Application>, sys: &sysinfo::System, users: &sysinfo::Users) -> Self {
        match category {
            Category::Name => {
                let icon_name = Some(match apps.iter().find(|app| {
                ...
                DataPoint { icon_name, data, category }
            }
            Category::User => {
                let user = users
                    ...
                    .name()
                    .into();
                DataPoint { icon_name: None, data: user, category }
            }
            Category::Cpu => {
                let data = format!("{:.1}%", process.cpu_usage() / sys.cpus().len() as f32);
                DataPoint { icon_name: None, data, category }
            }
            Category::Memory => {
                let data = {
